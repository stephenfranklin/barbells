...
#text(barb.tr$finalModel, use.n=TRUE, all=TRUE, cex=1)
...
#head(sort(barb.tr$finalModel$variable.importance, decreasing = T),10)

#barb.tr.imp<-as.list(sort(barb.tr$finalModel$variable.importance, decreasing = T))
#barb.tr.imp
...
#### The Random Forest took too long to calculate proximities with the 80% training set and with `prox=TRUE`.
```{r, eval=FALSE}
####  Train with 80%
set.seed(42)
#### barb80.rf : 
...
#### This Random Forest model used a training set of 20% to keep the processing time down (prox = TRUE).
...
The out-of-bag error rate is 2.4%. 
```{r model, dependson='split20', cache=TRUE}
set.seed(42)
...
#### Predicting with the test set of 80% 
```{r}
pred <- predict(barb.rf,testing80)
testing$predRight <- pred==testing80$classe
table(pred,testing80$classe)
```

#### This Random Forest model uses an 80% Training set but doesn't calculate proximities.

The features' importance branches differently. It attains an out-of-bag error rate of 0.5% and an out-of-sample error rate of .
```{r, dependson='split', cache=TRUE}
...
rownames(imp80)<-NULL
head(imp[with(imp80, order(-MeanDecreaseGini)), ],10)
